...
def is_valid_start_end(start_index_cand, end_index_cand, id_lists, tokenizer):
    ...
    if start_index_cand<=end_index_cand and end_index_cand>=sep_pos+1:
        if start_index_cand<=sep_pos:
            start_index_cand = sep_pos + 1
        is_valid = True 
        ans_text = tokenizer.decode(id_lists[start_index_cand:end_index_cand+1])
    return is_valid, ans_text

def demo(question="Who was Jim Henson ?", text="Jim Henson was a nice puppet jflsjflsjflsuu"):
...
def predict_doc(question, doc_text):
    inputs = tokenizer.encode_plus(question, doc_text)
    inputs = {
                "input_ids": torch.tensor([inputs["input_ids"]], device="cuda"),
                "attention_mask": torch.tensor([inputs["attention_mask"]], device="cuda"),
                "token_type_ids": torch.tensor([inputs["token_type_ids"]], device="cuda"),
            }
    start_scores, end_scores = model(**inputs)
    start_index = torch.argmax(start_scores)
    end_index = torch.argmax(end_scores)
    has_ans, ans_text = is_valid_start_end(start_index, end_index, inputs["input_ids"].cpu().tolist()[0], tokenizer)
...
def test_doc_evaluate_with_overlap_f1(test_file_path, verbose=False):
    detail_output_path = test_file_path+"_bert_qa_prediction_detail_squad_writing.txt"
    y_label = []
    y_hat_list = []
    dataset = load_test_doc_data(test_file_path)
    overlap_f1_list = []
