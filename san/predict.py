...
checkpoint_dict = torch.load("../checkpoint/checkpoint_best.pt") 
...
def main():
    ...
    # checkpoint_dict["config"]["v2_on"]=True
    model = DocReaderModel(opt, embedding, state_dict =checkpoint_dict["state_dict"])
    # model meta str
    ...
    if args.v2_on:
        ...
        logger.warning("dev EM: {:.3f} F1: {:.3f}  ACC: {:.3f}".format( em, f1, acc ))
    else:
